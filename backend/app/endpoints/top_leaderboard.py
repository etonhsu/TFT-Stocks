...
async def get_top_leaderboard(db: Session = Depends(get_db)):
    try:
        ...
        response = TopLeaderboard(
            price=TopLeaderboardEntry(name=price_data[0].game_name, value=price_model(float(price_data[1]))) if price_data else None,
            delta_8h=TopLeaderboardEntry(name=top_8h.game_name, value=top_8h.delta_8h) if top_8h else None,
            delta_24h=TopLeaderboardEntry(name=top_24h.game_name, value=top_24h.delta_24h) if top_24h else None,
            delta_72h=TopLeaderboardEntry(name=top_72h.game_name, value=top_72h.delta_72h) if top_72h else None,
